...
@click.option(
    ...
    default=["BTCUSDT"],
    show_default=False,
    help="Enter multiple crypto symboles ex. -s BTCUSDT -s ETHUSDT"
    )
@click.option(
    "-c",
    "--currency",
...
@pass_environment
def cli(ctx, symbols, refresh, currency, decimals):
    ...
    while True:
        for s in symbols:
            ctx.vlog(click.style(symbols, fg="bright_red"))
            binance(s)   
            ...
            ctx.log(click.style(str(f"{currency}{round(float(ctx.json_data['priceChange']),decimals)} {round(float(ctx.json_data['priceChangePercent']),decimals)}%"), fg="bright_magenta"))
